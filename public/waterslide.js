$(function() {
	...
	button.click(function(e) {
		...
		var dialog = $("<div id='waterslide_dialog'><div class='close'>X</div><div class='content'></div><button id='waterslide_send'>Send Feedback</button></div>").appendTo("body");
		dialog.css("top", $(window).height() / 2 - (dialog.height() / 2) + "px").css("left", $(window).width() / 2 - (dialog.width() / 2) + "px");
		...
		content.append("<label for='waterslide_email'>Your Email (Optional)</label><input type='text' id='waterslide_email' name='email' />")
	...
	$("#waterslide_send").live("click", function(e) {
		...
		$.post(endpoint, {
			...
			if(data == "success") {
				...
				$("#waterslide_send").text("Feedback Sent!")
			}
